...
- The full mode adds machine-validated theorem-frontier worker/reviewer artifacts, a structural paper-verifier gate, an authoritative local theorem DAG, blocker/cone metrics, and active-edge escalation tracking.
- The full mode uses two structural verifier roles: the paper-verifier checks paper-faithfulness of structural edits, and the NL-proof verifier checks the rigor of the natural-language proofs carried by newly admitted edges and leaf nodes.
...
- In full mode, edges are authoritative too: every edge gets a canonical `edge_id = parent|edge_type|child` plus an edge `status`, and structural approval / history replay use those ids rather than loose parent-child matching.
- In full mode, the active frontier item is an obligation edge (`active_edge_id`), not a theorem node. The active theorem node is now derived from the parent of that edge and retained only as contextual information.
- The intended proof semantics are edge-centric. A productive cycle should either close one or more existing edges or expand one unresolved edge into a richer local sub-DAG. Non-leaf node proof closure is derived from closed outgoing dependency edges to proved child nodes; the web DAG therefore uses effective proof status, not just raw workflow node status, when deciding what is green.
- Direct theorem proving is represented by explicit `direct_proof` self-edges. Those edges must be admitted like any other edge and therefore carry canonical ids plus natural-language proofs.
- In full mode, successful structural admission is proof-carrying: every newly admitted edge must include a complete natural-language proof of that implication/reduction, and every newly admitted leaf node must include a complete natural-language proof of the leaf theorem itself.
...
- In full mode, `theorem_stating` must also write `.agent-supervisor/paper_main_results.json`, a machine-readable coarse paper-DAG manifest. The `theorem_stating -> proof_formalization` transition validates that manifest and seeds the initial theorem-frontier DAG from it, so proof formalization starts from an explicit paper-derived proof spine rather than a single top-level node.
...
- In full mode, branching artifacts are anchored to the current theorem-frontier active obligation edge via `frontier_anchor_edge_id` (with legacy `frontier_anchor_node_id` accepted for compatibility).
- In full mode, a new child branch inherits the authoritative DAG itself but resets local frontier-aging/escalation pressure (`active_edge_age`, `active_leaf_age`, blocker age, failed-close streak, cone-purity streak, escalation, and last frontier artifacts), so fresh branches do not inherit parent stagnation pressure.
...
- Branch monitoring snapshots now include theorem-frontier summaries for each child branch: current active edge, current active theorem node, blocker cluster, open-hypothesis count, frontier ages, and escalation state.
