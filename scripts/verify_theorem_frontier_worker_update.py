...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate a theorem-frontier worker update with the supervisor's exact worker-stage checks."
    )
    parser.add_argument("--config", help="Path to the supervisor JSON config.")
    parser.add_argument(
        "--repo",
        help="Repo root; used only to find .agent-supervisor/config_path.txt when --config is omitted.",
    )
    ...
    try:
        ...
        report = supervisor.verify_theorem_frontier_worker_update_artifact(
            ...
            cycle_baseline=supervisor.current_cycle_lean_baseline(state, cycle),
        )
